...
pub fn get_target_scale(target: DynamicImage, side_len: usize) -> Vec<Vec<(u32, u8)>> {
...
pub fn preview(target: DynamicImage, version: Version, brightness_threshold: u8) -> ImageBuffer<Rgb<u8>, Vec<u8>> {
    let side_len = crate::consts::side_len_of_version(version);

    let brightness = make_brightness_array(target);

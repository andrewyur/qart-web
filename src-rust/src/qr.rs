...
pub fn preview(
    ...
    let target = target::get_target_scale(image, side_len);
    ...
    let mut result: ImageBuffer<Rgba<u8>, Vec<u8>> = image::ImageBuffer::new(side_len as u32, side_len as u32);
...
pub fn build(
    ...
    let target_arr = target::get_target_scale(image, side_length as usize);
    ...
    module_info.iter().for_each(|module| {
        blocks[module.block_index].set(module.bit_index, (module.target_color == module.mask) as u8);
    });
